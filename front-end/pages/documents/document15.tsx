...
type Props = NativeStackScreenProps<RootStackParamList, 'Document15'>;

const Document15: React.FC<Props> = ({ navigation }) => {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.progressContainer}>
        ...
        {[...Array(5)].map((_, index) => (
          ...
          <View style={[styles.progressStep, index === 4 && styles.activeStep]}>
            <Text style={[styles.progressText, index === 4 && styles.activeProgressText]}>{index + 11}</Text>
          </View>
        </View>
        ))}
        <TouchableOpacity onPress={() => navigation.navigate('DocumentFinish')}>
          <Icon name="chevron-forward" size={28} color="#7d0a16" />
        </TouchableOpacity>
      </View>

      <View style={styles.documentationInfo}>
        <Text style={styles.subtitle}>Termo de adesão e responsabilidade ao Programa UG</Text>
        <Text style={styles.explanation}>
            Faça o download do arquivo de termo de adesão e responsabilidade ao programa Universidade, preencha o mesmo, reconheça sua assinatura em cartório e anexe o mesmo nessa aba. Verifique o ícone de ajuda para mais detalhes
        </Text>
        <TouchableOpacity
    style={styles.helpIcon}
    onPress={() => {
      Alert.alert('Ajuda', 'Preenchimento do documento conforme modelo disponibilizado pela Católica SC, onde o acadêmico irá se comprometer a participar do Programa Universidade Gratuita conforme legislação vigente, bem como se responsabilizará por todas as informações prestadas e documentos encaminhados para a Católica de Santa Catarina. O documento deverá ser assinado pelo acadêmico e ter o reconhecimento da assinatura em cartório ou assinatura digital pelo aplicativo GOV.');
    }}
  >
    <Icon name="help-circle-outline" size={24} color="#7d0a16" />
      ...
      <View style={styles.documentContainer}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('Document14')}>
            <Icon name="chevron-back" size={28} color="#7d0a16" />
            <Text style={styles.buttonText}>Voltar</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('DocumentFinish')}>
      ...
      <View style={styles.bottomBarContainer}>
        <AppBottomBar currentTab="Document15" />
      </View>
    </View>
  );
};
