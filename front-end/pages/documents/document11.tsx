...
type Props = NativeStackScreenProps<RootStackParamList, 'Document11'>;

const Document11: React.FC<Props> = ({ navigation }) => {
      ...
      <View style={styles.progressContainer}>
        ...
        {[...Array(5)].map((_, index) => (
          ...
          <View style={[styles.progressStep, index === 0 && styles.activeStep]}>
            <Text style={[styles.progressText, index === 0 && styles.activeProgressText]}>{index + 11}</Text>
          </View>
        </View>
        ))}
        <TouchableOpacity onPress={() => navigation.navigate('Document12')}>
          <Icon name="chevron-forward" size={28} color="#7d0a16" />
        </TouchableOpacity>
      </View>

      <View style={styles.documentationInfo}>
        <Text style={styles.subtitle}>Comprovante de despesas familiar com transporte coletivo para estudo</Text>
        <Text style={styles.explanation}>
        Envie abaixo o comprovante de despesa familiar mensal para a finalidade de estudar, com transporte coletivo. Somente para quem declarou no formulário de cadastro que possui este tipo de despesa mensal
        </Text>
        <TouchableOpacity
    style={styles.helpIcon}
    onPress={() => {
      Alert.alert('Ajuda', 'a) no caso de utilização de transporte coletivo, apresentar cópia do último recibo de compra da passagem de ônibus ou detalhamento de recarga do cartão de uso no transporte público, onde conste a data da última recarga. O recibo ou detalhamento de recarga, deve conter os dados da empresa de transporte coletivo, inclusive o CNPJ da mesma e o nome do acadêmico ou membro do grupo familiar que possui a despesa mensal com transporte público, para fins de locomoção até o local de estudos e o valor mensal do gasto com transporte coletivo, conforme informado no cadastro, pelo candidato ao benefício; \nb) no caso de utilização de transporte terceirizado (Topic/Van), apresentar o recibo de pagamento referente ao último mês de utilização do transporte, ou contrato com a empresa que presta o serviço de transporte. O recibo ou contrato da empresa de transporte deve conter os dados da mesma, inclusive o CNPJ e assinatura da empresa e do usuário do transporte.');
    }}
  >
    <Icon name="help-circle-outline" size={24} color="#7d0a16" />
      ...
      <View style={styles.documentContainer}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('Document12')}>
      ...
      <View style={styles.bottomBarContainer}>
        <AppBottomBar currentTab="Document11" />
      </View>
    </View>
  );
};
