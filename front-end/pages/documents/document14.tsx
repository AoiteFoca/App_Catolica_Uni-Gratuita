...
type Props = NativeStackScreenProps<RootStackParamList, 'Document14'>;

const Document14: React.FC<Props> = ({ navigation }) => {
      ...
      <View style={styles.progressContainer}>
        ...
        {[...Array(6)].map((_, index) => (
          ...
          <View style={[styles.progressStep, index === 4 && styles.activeStep]}>
            <Text style={[styles.progressText, index === 4 && styles.activeProgressText]}>{index + 10}</Text>
          </View>
        </View>
        ))}
        <TouchableOpacity onPress={() => navigation.navigate('Document15')}>
          <Icon name="chevron-forward" size={28} color="#7d0a16" />
        </TouchableOpacity>
      </View>

      <View style={styles.documentationInfo}>
        <Text style={styles.subtitle}>Comprovante de despesas familiar mensal com educação regular paga para outro membro do grupo familiar</Text>
        <Text style={styles.explanation}>
            Caso algum membro do grupo familiar esteja estudando em instituição particular, envie o comprovante de matrícula e o comprovante de pagamento da última mensalidade
        </Text>
        <TouchableOpacity
    style={styles.helpIcon}
    onPress={() => {
      Alert.alert('Ajuda', 'Se algum membro do grupo familiar do acadêmico estiver estudando em Instituição particular de ensino regular, independente do grau de ensino (infantil, básica ou superior), o acadêmico deverá apresentar a confirmação de matrícula ou declaração de matrícula + o comprovante de pagamento da última mensalidade.');
    }}
  >
    <Icon name="help-circle-outline" size={24} color="#7d0a16" />
      ...
      <View style={styles.documentContainer}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('Document15')}>
      ...
      <View style={styles.bottomBarContainer}>
        <AppBottomBar currentTab="Document14" />
      </View>
    </View>
  );
};
