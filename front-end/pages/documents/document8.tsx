...
type Props = NativeStackScreenProps<RootStackParamList, 'Document8'>;

const Document8: React.FC<Props> = ({ navigation }) => {
      ...
      <View style={styles.progressContainer}>
        ...
        {[...Array(6)].map((_, index) => (
          ...
          <View style={[styles.progressStep, index === 2 && styles.activeStep]}>
            <Text style={[styles.progressText, index === 2 && styles.activeProgressText]}>{index + 6}</Text>
          </View>
        </View>
        ))}
        <TouchableOpacity onPress={() => navigation.navigate('Document9')}>
          <Icon name="chevron-forward" size={28} color="#7d0a16" />
        </TouchableOpacity>
      </View>

      <View style={styles.documentationInfo}>
        <Text style={styles.subtitle}>Comprovante de despesa com habitação (aluguel ou financiamento)</Text>
        <Text style={styles.explanation}>
            Envie abaixo os recibos de pagamento do último mês, dependendo do seu tipo de moradia (verificar no ícone de ajuda)
        </Text>
        <TouchableOpacity
    style={styles.helpIcon}
    onPress={() => {
      Alert.alert('Ajuda', 'a)No caso de imóvel alugado - Apresentar cópia do recibo de pagamento do último mês aluguel + comprovante de residência atualizado referente ao último mês.\nb) Financiamento - Apresentar o recibo da última parcela de pagamento do financiamento, que poderá ser o extrato bancário do período no caso de débito automático + comprovante de residência atualizado referente ao último mês.');
    }}
  >
    <Icon name="help-circle-outline" size={24} color="#7d0a16" />
      ...
      <View style={styles.documentContainer}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('Document9')}>
      ...
      <View style={styles.bottomBarContainer}>
        <AppBottomBar currentTab="Document8" />
      </View>
    </View>
  );
};
