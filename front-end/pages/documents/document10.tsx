...
type Props = NativeStackScreenProps<RootStackParamList, 'Document10'>;

const Document10: React.FC<Props> = ({ navigation }) => {
      ...
      <View style={styles.progressContainer}>
        ...
        {[...Array(5)].map((_, index) => (
          ...
          <View style={[styles.progressStep, index === 4 && styles.activeStep]}>
            <Text style={[styles.progressText, index === 4 && styles.activeProgressText]}>{index + 6}</Text>
          </View>
        </View>
        ))}
        <TouchableOpacity onPress={() => navigation.navigate('Document11')}>
          <Icon name="chevron-forward" size={28} color="#7d0a16" />
        </TouchableOpacity>
      </View>

      <View style={styles.documentationInfo}>
        <Text style={styles.subtitle}>Comprovante do Ensino Médio</Text>
        <Text style={styles.explanation}>
            Envie abaixo o histórico escolar do ensino médio/certificado. Em caso de bolsistas, apresentar a declaração que comprove o recebimento da bolsa
        </Text>
        <TouchableOpacity
    style={styles.helpIcon}
    onPress={() => {
      Alert.alert('Ajuda', 'Apresentar histórico escolar do ensino médio/certificado. No caso dos estudantes que tenham cursado o ensino médio na condição de bolsista integral ou parcial, deverão apresentar também, a declaração que comprove o recebimento de tal bolsa.');
    }}
  >
    <Icon name="help-circle-outline" size={24} color="#7d0a16" />
      ...
      <View style={styles.bottomBarContainer}>
        <AppBottomBar currentTab="Document10" />
      </View>
    </View>
  );
};
