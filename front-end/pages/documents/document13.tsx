...
type Props = NativeStackScreenProps<RootStackParamList, 'Document13'>;

const Document13: React.FC<Props> = ({ navigation }) => {
      ...
      <View style={styles.progressContainer}>
        ...
        {[...Array(6)].map((_, index) => (
          ...
          <View style={[styles.progressStep, index === 3 && styles.activeStep]}>
            <Text style={[styles.progressText, index === 3 && styles.activeProgressText]}>{index + 10}</Text>
          </View>
        </View>
        ))}
        <TouchableOpacity onPress={() => navigation.navigate('Document14')}>
          <Icon name="chevron-forward" size={28} color="#7d0a16" />
        </TouchableOpacity>
      </View>

      <View style={styles.documentationInfo}>
        <Text style={styles.subtitle}>Comprovante de deficiência ou invalidez permanente</Text>
        <Text style={styles.explanation}>
            Envie abaixo os documentos que comprovem a deficiência ou invalidez permanente, de acordo com os tipos aceitos citados no ícone de ajuda
        </Text>
        <TouchableOpacity
    style={styles.helpIcon}
    onPress={() => {
      Alert.alert('Ajuda', 'Atestado médico, atestando a deficiência “irreversível”, ou invalidez permanente do acadêmico, onde conste o CID relativo à deficiência. Este atestado será utilizado para finalidade de assistência financeira do Programa Universidade Gratuita e é solicitado de acordo com a legislação vigente');
    }}
  >
    <Icon name="help-circle-outline" size={24} color="#7d0a16" />
      ...
      <View style={styles.bottomBarContainer}>
        <AppBottomBar currentTab="Document13" />
      </View>
    </View>
  );
};
